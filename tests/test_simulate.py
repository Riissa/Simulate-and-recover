...
class TestSimulateObservedData(unittest.TestCase):
    ...
    def test_noise_effects(self):
        ...
        R_obs_small, M_obs_small, V_obs_small = simulate_observed_data(R_pred, M_pred, V_pred, trial_num=3, N=10)

        # Simulate with large N (less noise) #idk what setting trial_num to 3 does
        R_obs_large, M_obs_large, V_obs_large = simulate_observed_data(R_pred, M_pred, V_pred, trial_num = 3, N=4000)
